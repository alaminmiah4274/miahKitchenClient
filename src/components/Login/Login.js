...
const Login = () => {
	// to get context data
	const { userLogIn, providerLogin, error, setError } =
	...
	const handleLogInForm = (e) => {
		...
		userLogIn(email, password)
			.then((result) => {
				...
				fetch("https://miah-kitchen-server.vercel.app/jwt", {
					method: "POST",
					headers: {
				...
				// navigate the user home after login
				navigate("/");
			})
	...
	return (
		<div className="hero min-h-screen bg-base-100">
			<div className="hero-content flex-col">
				...
				<div className="card flex w-[300px] md:w-[500px] bg-white">
					...
					<p className="text-center mb-5 text-xs md:text-sm">
						New to Miah Kitchen?{" "}
					...
					<button
						...
						className="border rounded-full w-3/5 mx-auto my-5 flex items-center"
					>
						<img src={googleLogo} alt="" width={40} />
						<p className="ms-3 text-xs md:text-sm ">
